...
splitData = function(Kfold, seed, datafr, threshhold=0.5, switchVar = 1, methodVar = "gbm")
{ set.seed(seed)
  n = nrow(datafr)
  iperm<<-sample(n) # set as global for debugging check
  nhold = round(n/Kfold)
  reg = list()
  pred = list() # is there a better way than lists in R?
  aucVar = list()
  rocVar = list()
